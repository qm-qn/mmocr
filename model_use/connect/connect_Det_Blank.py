...
def delete_bottom_lines(editable_pixels, width, height, line_to_white, white_color, word_black_color):
    ...
    for y in range(height):
        ...
        for x in range(width):
            if int(sum(editable_pixels[x, y]) / 3) < word_black_color:
...
def find_black_line(editable_pixels, width, height):
    ...
    for y in range(height):
        ...
        # 统计当前行的黑色像素数
        for x in range(width):
            if int(sum(editable_pixels[x, y]) / 3) < line_black_color:
...
def delete_blank(image_path, output_path):
    # 打开图像
    ...
    for x in range(width):
        # 统计当前列的黑色像素数
        is_blank_black_pixel_count = sum(1 for y in range(height) if int(sum(editable_pixels[x, y]) / 3) < black_color)
    ...
    if is_blank == 1:
        ...
        for x in range(width):
            ...
            black_pixel_count2 = sum(1 for y in range(height) if int(sum(editable_pixels[x, y]) / 3) < black_color)
        ...
        for y in range(height):
            # 统计当前行的黑色像素数
            top_black_pixel_count = sum(1 for x in range(left_bound, right_bound + 1) if int(sum(editable_pixels[x, y]) / 3) < black_color)
        ...
        for y in range(height - 1, 0, -1):
            # 统计当前行的黑色像素数
            bottom_black_pixel_count = sum(1 for x in range(left_bound, right_bound + 1) if int(sum(editable_pixels[x, y]) / 3) < black_color)
