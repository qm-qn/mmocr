...
def delete_stroke(editable_pixels, width, height, line_y, top_y_end, top_black, bottom_y_end, bottom_black, white_color):
    global is_black
    global to_white
    global connect
    global is_find
    ...
    #             # 底部黑点列表去掉要去除的黑点
    #             if n > bottom_y_end:
    #                 for i in range(len(bottom_black)):
    #                     if bottom_black[i] == (m, n):
    #                         bottom_black.pop(i)
    #                         break
    #             editable_pixels[m, n] = white_color
    #     to_white.clear()
...
def crop_image(image_path, file_save_folder, file_view_folder, noline_path):
    # 打开图像
    ...
    for y in range(height):
        ...
        # 统计当前行的黑色像素数
        for x in range(width):
            gray_value = editable_pixels[x, y]
...
def process_images_in_folder(folderIn_path, folder_save_folder, folder_view_folder):
    ...
    for filename in os.listdir(folderIn_path):
        ...
        if os.path.isdir(fileInpath):
            ...
            # 如果是图片文件，则进行处理
            crop_image(fileInpath, file_save_folder, file_view_folder, noline_path)
            # 更新进度条
